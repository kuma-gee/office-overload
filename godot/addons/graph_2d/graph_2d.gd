...
func _ready():
	...
	axis.default_color = default_color
	add_child(axis)
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		if plot_rect.has_point(plot_area.get_local_mouse_position()):
			...
			if first_plot:
				var p = first_plot.find_point(point, 0.2)
				if p:
					first_plot.set_active_point(p)
...
func add_plot_item(label = "", color = Color.WHITE, width = 1.0) -> PlotItem:
	...
	if not first_plot:
		first_plot = plot
		first_plot.coordinate = coordinate
...
func _update_graph() -> void:
	...
	var margin_left: float = _MARGIN_LEFT #if get_node("Axis").y_label == "" else _MARGIN_LEFT + 10
	var margin_bottom: float = _MARGIN_BOTTOM #if get_node("Axis").x_label == "" else _MARGIN_BOTTOM + 10
	...
	for n in range(vert_grad_number):
		...
		grad.append(grad_px)
		var grad_text = "%0.0f" % (float(y_max) - n * float(y_axis_range)/(vert_grad_number-1))
	...
	for n in range(hor_grad_number):
		...
		grad.append(grad_px)
		var grad_text = "%0.0f" % (float(x_min) + n * float(x_axis_range)/(hor_grad_number-1))
		grad.append(grad_text)
