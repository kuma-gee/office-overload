...
func _ready():
	...
	#x_label_node.add_theme_font_size_override("font_size", default_font_size)
	#add_child(x_label_node)
	#var y_label_node = Label.new()
	#y_label_node.name = "YLabel"
	#y_label_node.rotation = -PI/2
	#y_label_node.add_theme_font_size_override("font_size", default_font_size)
...
func _draw() -> void:
	...
	var offset = 0
	var topleft: Vector2 = vert_grad.front()[POINT] + Vector2(offset, 0)
	var topright: Vector2 = Vector2(hor_grad.back()[POINT].x, vert_grad.front()[POINT].y)
	var bottomright: Vector2 = hor_grad.back()[POINT] + Vector2(offset, 0)
	...
	if show_horizontal_line:
		draw_line(hor_grad.front()[POINT] + Vector2(offset, 0), hor_grad.back()[POINT], default_color, 1)

	if show_y_ticks:
		for grad in vert_grad:
			draw_line(grad[POINT] + Vector2(offset, 0), grad[POINT] - Vector2(5, 0), default_color, 1)
	
	if show_y_numbers:
		for grad in vert_grad:
			draw_string(default_font, grad[0] + Vector2(-20, 2.5), grad[1], HORIZONTAL_ALIGNMENT_CENTER, -1, default_font_size, default_color)
		
	if show_vertical_line:
		draw_line(topleft, vert_grad.back()[POINT] + Vector2(offset, 0), default_color, 1)
